...
export class StorageManager {
  ...
  private syncCache: Map<string, unknown> = new Map();
  ...
  async getSyncData<T>(key: string): Promise<T | null> {
    try {
      ...
      logger.error(`Failed to get sync data for key ${key}:`, error);
  ...
  async setSyncData<T>(key: string, data: T): Promise<void> {
    try {
      ...
      logger.debug(`Successfully set sync data for key: ${key}`);
    } catch (error) {
      logger.error(`Failed to set sync data for key ${key}:`, error);
      throw new StorageError(
  ...
  async getLocalData<T>(key: string): Promise<T | null> {
    try {
      ...
      logger.error(`Failed to get local data for key ${key}:`, error);
      throw new StorageError(
  ...
  async setLocalData<T>(key: string, data: T): Promise<void> {
    try {
      ...
      logger.debug(`Successfully set local data for key: ${key}`);
    } catch (error) {
      logger.error(`Failed to set local data for key ${key}:`, error);
      throw new StorageError(
  ...
  async removeSyncData(key: string): Promise<void> {
    try {
      ...
      logger.debug(`Successfully removed sync data for key: ${key}`);
    } catch (error) {
      logger.error(`Failed to remove sync data for key ${key}:`, error);
      throw new StorageError(
        STORAGE_ERROR_CODES.SYNC_WRITE_FAILED,
  ...
  async removeLocalData(key: string): Promise<void> {
    try {
      ...
      logger.debug(`Successfully removed local data for key: ${key}`);
    } catch (error) {
      logger.error(`Failed to remove local data for key ${key}:`, error);
      throw new StorageError(
        STORAGE_ERROR_CODES.LOCAL_WRITE_FAILED,
  ...
  async getSyncQuotaInfo(): Promise<QuotaInfo> {
    try {
      ...
      logger.error('Failed to get sync quota info:', error);
      throw new StorageError(
        STORAGE_ERROR_CODES.SYNC_READ_FAILED,
  ...
  async getLocalQuotaInfo(): Promise<QuotaInfo> {
    try {
      ...
      logger.error('Failed to get local quota info:', error);
      throw new StorageError(
        STORAGE_ERROR_CODES.LOCAL_READ_FAILED,
  ...
  private async checkLocalQuota<T>(key: string, data: T): Promise<void> {
    ...
    if (localQuota.used + dataSize > localQuota.available * 0.8) {
      logger.warn(
        `Writing ${dataSize} bytes would exceed 80% of local quota, attempting emergency cleanup`
      );
  ...
  private async emergencyCleanup(): Promise<boolean> {
    try {
      logger.info('Attempting emergency cleanup of local storage');
      ...
      if (cleanedSize > 0) {
        ...
        logger.info(`Emergency cleanup freed approximately ${cleanedSize} bytes`);
        return true;
      }
